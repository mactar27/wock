...
async function getSmartphones(): Promise<Product[]> {
  try {
    const products = await query(
      "SELECT * FROM products WHERE category = 'smartphone' ORDER BY created_at DESC"
    ) as Product[];
    
    return products || [];
  } catch (error) {
    console.error("Error fetching smartphones from MySQL:", error);
    return [];
  }
}
      ...
      <section className="pt-40 pb-16 bg-background">
        <div className="mx-auto max-w-7xl px-6 text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-foreground leading-[1.1]">
            Smartphones <span className="text-primary">&</span> Mobiles
          </h1>
          <p className="mt-6 text-xl text-muted-foreground max-w-2xl mx-auto">
            Une sélection rigoureuse des meilleurs terminaux du marché, pour tous les usages.
          </p>
        </div>
      </section>
